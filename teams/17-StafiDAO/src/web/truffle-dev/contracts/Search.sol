...
interface IFaucet {
    ...
    function getRecordRewardDate() external view returns (uint256);

    function getPendingRedeemDate() external view returns (uint256);
}
...
interface IGovernance {
    ...
    function getRewardDownLimit() external view returns (uint256);
...
contract Search {
    ...
    function isScheduleRedeemStake(address _faucetAddr)
        public
        view
        returns (bool)
    {
        return
            IFaucet(_faucetAddr).bstate() &&
            IFaucet(_faucetAddr).leaveNumber() == 0 &&
            IFaucet(_faucetAddr).getPendingRedeemDate() > 0;
    }

    function isExecuteRedeemStake(address _faucetAddr)
        public
        view
        returns (bool)
    {
        return
            IFaucet(_faucetAddr).leaveNumber() > 0 &&
            IFaucet(_faucetAddr).leaveNumber() + Igovern.blockHeight() <=
            block.number;
    }

    function isZeroIncomePunish(address _faucetAddr)
        public
        view
        returns (bool)
    {
    ...
    function isRecordRewardInfo(address _faucetAddr)
        public
        view
        returns (bool)
    {
        return
            IFaucet(_faucetAddr).bstate() &&
            IFaucet(_faucetAddr).getRecordRewardDate() <
