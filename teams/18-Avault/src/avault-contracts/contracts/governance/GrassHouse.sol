...
import "../interfaces/IveAVAT.sol";
import "../libraries/Math128.sol";

/// @title GrassHouse - provide rewards for veAVAT
...
contract GrassHouse is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  address public veAVAT;
  address public rewardToken;
  uint256 public lastTokenBalance;

  /// @dev veAVAT supply at week bounds
  ...
  /// @param _veAVAT The address of veAVAT
  ...
  /// @notice Get veAVAT balance of "_user" at "_timstamp"
  ...
  function balanceOfAt(address _user, uint256 _timestamp) external view returns (uint256) {
    uint256 _maxUserEpoch = IveAVAT(veAVAT).userPointEpoch(_user);
    if (_maxUserEpoch == 0) {
      return 0;
    }

    uint256 _epoch = _findTimestampUserEpoch(_user, _timestamp, _maxUserEpoch);
    Point memory _point = IveAVAT(veAVAT).userPointHistory(_user, _epoch);
  ...
  function _checkpointToken() internal {
    ...
    uint256 _rewardTokenBalance = IERC20Upgradeable(rewardToken).balanceOf(address(this));
  ...
  function _checkpointTotalSupply() internal {
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      if (_weekCursor > _roundedTimestamp) {
        ...
        Point memory _point = IveAVAT(veAVAT).pointHistory(_epoch);
        int128 _timeDelta = 0;
  ...
  /// @notice Update veAVAT total supply checkpoint
  ...
  function _claim(address _user, uint256 _maxClaimTimestamp) internal returns (uint256) {
    ...
    Point memory _userPoint = IveAVAT(veAVAT).userPointHistory(_user, _userEpoch);
    ...
    // This can be:
    // 1) User just lock their AVAT less than 1 week
    // 2) User already claimed their rewards
    if (_userWeekCursor >= _maxClaimTimestamp) {
      return 0;
    }

    // Handle when user lock AVAT before Grasshouse started
    ...
    for (uint256 _i = 0; _i < 52; _i++) {
      ...
      if (_userWeekCursor >= _userPoint.timestamp && _userEpoch <= _maxUserEpoch) {
        ...
        if (_userEpoch > _maxUserEpoch) {
          ...
          _userPoint = IveAVAT(veAVAT).userPointHistory(_user, _userEpoch);
        }
      } else {
  ...
  function claim(address _user) external nonReentrant onlyLive returns (uint256) {
    ...
    if (_amount != 0) {
      ...
      IERC20Upgradeable(rewardToken).safeTransfer(_user, _amount);
    }
  ...
  function claimMany(address[] calldata _users) external nonReentrant onlyLive returns (bool) {
    ...
    for (uint256 i = 0; i < _users.length; i++) {
      ...
      if (_amount != 0) {
        IERC20Upgradeable(rewardToken).safeTransfer(_users[i], _amount);
  ...
  function _findTimestampEpoch(uint256 _timestamp) internal view returns (uint256) {
    uint256 _min = 0;
    uint256 _max = IveAVAT(veAVAT).epoch();
    ...
    for (uint256 i = 0; i < 128; i++) {
      ...
      Point memory _point = IveAVAT(veAVAT).pointHistory(_mid);
  ...
  function _findTimestampUserEpoch(
    ...
    for (uint256 i = 0; i < 128; i++) {
      ...
      uint256 _mid = (_min + _max + 1) / 2;
      Point memory _point = IveAVAT(veAVAT).userPointHistory(_user, _mid);
      if (_point.timestamp <= _timestamp) {
        _min = _mid;
      } else {
        _max = _mid - 1;
  ...
  function kill() external onlyOwner {
    isKilled = true;
    IERC20Upgradeable(rewardToken).safeTransfer(emergencyReturn, IERC20Upgradeable(rewardToken).balanceOf(address(this)));
