...
/// @title veAVAT - The goverance token of AVAT, inspired by veAVAT and Pancake pool
// solhint-disable not-rely-on-time
// solhint-disable-next-line contract-name-camelcase
contract veAVAT is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  uint256 public constant MAX_LOCK = (157 * WEEK) - 1;
  uint256 public constant MULTIPLIER = 10**18;

  // Token to be locked (AVAT)
  address public token;
  // Total supply of AVAT that get locked
  ...
  /// @notice Initialize veAVAT
  /// @param _token The address of AVAT token
  function initialize(address _token) external initializer {
    ...
    name = "vote escrow AVAT";
    symbol = "veAVAT";
  }


  /// @notice Return the balance of veAVAT at a given "_blockNumber"
  /// @param _user The address to get a balance of veAVAT
  /// @param _blockNumber The speicific block number that you want to check the balance of veAVAT
    ...
    if (_address != address(0)) {
      ...
      if (_newLocked.end != 0) {
        ...
        if (_newLocked.end == _prevLocked.end) {
          ...
          // Possibly when user deposited more AVAT to his locker
  ...
  /// @dev This will crate a new lock and deposit AVAT to veAVAT Vault
  /// @param _amount the amount that user wishes to deposit
  /// @param _unlockTime the timestamp when AVAT get unlocked, it will be
  ...
  /// @notice Internal function to perform deposit and lock AVAT for a user
  /// @param _for The address to be locked and received veAVAT
  /// @param _amount The amount to deposit
  /// @param _unlockTime New time to unlock AVAT. Pass 0 if no change.
  ...
  /// @param _amount The amount of AVAT to be added to the lock
  ...
  /// @notice Calculate total supply of veAVAT (voting power)
  function totalSupply() external view returns (uint256) {
    return _totalSupplyAt(pointHistory[epoch], block.timestamp);
  }

  /// @notice Calculate total supply of veAVAT at specific block
  ...
  /// @notice Calculate total supply of veAVAT (voting power) at some point in the past
  ...
  /// @notice Withdraw all AVAT when lock has expired.
  ...
  function earlyWithdraw(uint256 _amount) external nonReentrant {
    ...
    // accumulate avat for redistribution
    accumRedistribute = accumRedistribute + _redistribute;

    // transfer one part of the penalty to treasury
    IERC20Upgradeable(token).safeTransfer(treasuryAddr, _penalty - _redistribute);
    // transfer remaining back to owner
    IERC20Upgradeable(token).safeTransfer(msg.sender, _amount - _penalty);
  ...
  function redistribute() external onlyRedistributors nonReentrant {
    ...
    IERC20Upgradeable(token).safeTransfer(redistributeAddr, _amount);
  ...
  function updateBoostContractInfo(address _user) internal {
      if (boostContract != address(0)) {
          IBoostContract(boostContract).onAVATPoolUpdate(
              _user
          );
      }
  }
  ...
  /**
    ...
    */
  function setBoostContract(address _boostContract) external onlyOwner {
