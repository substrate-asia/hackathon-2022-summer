...
abstract contract BaseNFT is ERC721Enumerable, IBaseNFT {
    ...
    // transfer lock (mapping of tokenId to bool)
    mapping(uint256 => bool) private _transferLock;

    function _setLock(uint256 tokenId, bool isLocked) internal {
        _transferLock[tokenId] = isLocked;
    }

    function mintToken(address owner, string memory contentUri, bool isLocked)
    internal
    returns (uint256)
    {
        _tokenIds.increment();
    ...
    function getTokenCount() public view returns (uint256) { return _tokenIds.current(); }
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        require(! _transferLock[tokenId], "Non Transferable");
    }
...
contract Profile is BaseNFT {
    ...
    string private constant PERM_ERR = "NOT ADMIN";
    ...
    string private constant RADIAN_DEFAULT_IMAGE = "https://d1skj8m4trskbx.cloudfront.net/misc/handleTpl.svg";
    string private constant PROFILE_HANDLE_ERR = "HANDLE TAKEN BY PROFILE";
    string private constant DOMAIN_ERR = "DOMAIN NOT APPROVED";

    uint8 private minHandleLength = 5;
    uint8 private maxHandleLength = 8;
    ...
    mapping(string=>bool) private _approvedDomain;

    struct HandlePaginationView {
        string[] results;
    ...
    modifier onlyAdmin {
        require(admin == msg.sender, PERM_ERR);
        _;
    }
    ...
    constructor(string memory tokenName, string memory symbol) ERC721(tokenName, symbol) {
        admin = msg.sender;
        _approvedDomain["RADI"] = true;
    }

    function setMin(uint8 min) external onlyAdmin {
        minHandleLength = min;
    }

    function setMax(uint8 max) external onlyAdmin {
        maxHandleLength = max;
    }

    function approveDomain(string memory domain) external onlyAdmin {
        _approvedDomain[domain] = true;
    }

    function disproveDomain(string memory domain) external onlyAdmin {
        _approvedDomain[domain] = false;
    }
    ...
    */
    ...
    returns (uint256)
    {
        ...
        uint256 profileId = BaseNFT.mintToken(msg.sender, contentUri, false);

        registerProfileHandle(profileId, profileHandle, domain);
    ...
    function registerProfileHandle(uint256 profileId, string memory handle, string memory domain) internal {
    ...
    function _getValidHandleHash(string memory handle, string memory domain) internal view returns (string memory, bytes32) {
        ...
        require(_approvedDomain[domain], DOMAIN_ERR);
    ...
    // Reference from Lens Protocol
    // source https://github.com/aave/lens-protocol
    function _validateHandle(string memory handle) private view {
        bytes memory byteHandle = bytes(handle);
        require(byteHandle.length >= minHandleLength && byteHandle.length <= maxHandleLength, HANDLE_LENGTH_ERR);

        uint256 byteHandleLength = byteHandle.length;
        for (uint256 i = 0; i < byteHandleLength; ) {
            require((
                (byteHandle[i] >= "0" && byteHandle[i] <= "9") ||
                (byteHandle[i] >= "A" && byteHandle[i] <= "Z") || 
                (byteHandle[i] >= "a" && byteHandle[i] <= "z") ||
                byteHandle[i] == "-" ||
                byteHandle[i] == "_"
            ), CHARATER_INVAID_ERR);
    ...
    // READ FUNCTION
    function getProfileFullHandle(uint256 profileId) public view returns (string memory) {
        return _stringOfHandleHash[_handleHashOfProfile[profileId]];
    }

    function getHandlesByAddress(address owner) external view returns (string[] memory) {
        uint256 ownedTokens = balanceOf(owner);
        string[] memory handles = new string[](ownedTokens);
        for ( uint i = 0 ; i < ownedTokens ; i++ ) {
            uint256 profileId = tokenOfOwnerByIndex(owner, i);
            handles[i] = _stringOfHandleHash[_handleHashOfProfile[profileId]];
        }
        return handles;
    }

    function getAddressByHandle(string memory handleName) external view returns (address) {
        bytes32 handleHash = keccak256(bytes(handleName));
        return ownerOf(_profileOfHandleHash[handleHash]);
    }

    function getHandlePaginatedList(
        uint256 offset,
        uint256 limit
    ) 
    public
    view
    returns (HandlePaginationView memory)
    {
        uint256 count = totalSupply();
        ( uint256 last, uint256 _limit ) = Pagination._paginationHandler(offset, limit, count);
        
        string[] memory handles = new string[](_limit);

        for (uint256 i = offset; i < last; i++) {
            handles[i - offset] = _stringOfHandleHash[_handleHashOfProfile[i+1]];
        }

        HandlePaginationView memory _handlePaginationView = HandlePaginationView(
            handles,
            Pagination._paginationResponseHandler(offset, limit, count)
        );
        
        return _handlePaginationView;
    }
